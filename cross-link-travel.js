(() => {
  const start = () => {
    if (window.ProfileCrossLinkTravel) return;
    const site = window.SITE_DATA;
    const graph = site?.graph;
    const scene = window.ProfileScene;
    const geometry = window.ProfileGlobalGeometry;
    if (!graph?.nodes?.length || !geometry) return;
    ...
    const emit = (name, detailPayload = {}) => window.dispatchEvent(new CustomEvent(`profile:crosslink-${name}`, { detail: { ...state, ...detailPayload } }));
    ...
    const relationsFor = sourceId => {
      ...
      return graph.edges
        ...
        .map(edge => {
          ...
          const vector = geometry.vectorBetween(sourceId, targetId);
          return {
            sourceId,
            targetId,
            target,
            ...
            family: copy.family,
            vector,
            direction: geometry.directionName(vector)
          };
        })
        .filter(Boolean)
        .sort((left, right) => {
          const priority = { evidence: 0, study: 1, experience: 2, related: 3 };
          return (priority[left.family] ?? 9) - (priority[right.family] ?? 9) || left.target.label.localeCompare(right.target.label);
        });
    };
    ...
    const introOwnsScreen = () => ['pending', 'waiting', 'running', 'identity', 'expanding', 'handoff'].includes(document.documentElement.dataset.profileIntro);
    ...
    const elementForNode = id => {
      ...
      if (id.startsWith('project-')) {
        ...
        return document.querySelector(`.work-project-anchor-v5[data-project-id="${projectId}"]`) || document.querySelector(`#site-graph .site-graph-node[data-node-id="${id}"]`);
      }
    ...
    const centerOf = (element, fallback = null) => {
      ...
      if (rect && rect.width >= 0 && rect.height >= 0) return { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2, rect };
      return fallback;
    };
    const fallbackGraphCenter = () => {
      const rect = document.querySelector('.site-graph-viewport')?.getBoundingClientRect();
      return rect ? { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2, rect } : { x: window.innerWidth / 2, y: window.innerHeight / 2, rect: null };
    };
    ...
    const portalFor = (source, vector) => {
      const marginX = Math.min(150, Math.max(76, window.innerWidth * .1));
      const marginY = Math.min(130, Math.max(70, window.innerHeight * .1));
      const candidates = [];
      if (vector.x > .05) candidates.push((window.innerWidth - marginX - source.x) / vector.x);
      if (vector.x < -.05) candidates.push((marginX - source.x) / vector.x);
      if (vector.y > .05) candidates.push((window.innerHeight - marginY - source.y) / vector.y);
      if (vector.y < -.05) candidates.push((marginY - source.y) / vector.y);
      const t = Math.max(1, Math.min(...candidates.filter(value => Number.isFinite(value) && value > 0)));
      return {
        x: clamp(source.x + vector.x * t, marginX, window.innerWidth - marginX),
        y: clamp(source.y + vector.y * t, marginY, window.innerHeight - marginY)
      };
    };
    ...
    const tracePath = (svg, from, to, className) => {
      ...
      const nx = -dy / length;
      const ny = dx / length;
      const bend = Math.min(46, length * .07);
      const c1 = { x: from.x + dx * .38 + nx * bend, y: from.y + dy * .38 + ny * bend };
      const c2 = { x: from.x + dx * .72 - nx * bend * .55, y: from.y + dy * .72 - ny * bend * .55 };
    ...
    const animateTrace = (path, traveller, duration, id) => new Promise(resolve => {
      ...
      if (reducedMotion.matches || duration <= 0) {
        ...
        if (traveller) { traveller.setAttribute('cx', finalPoint.x); traveller.setAttribute('cy', finalPoint.y); }
        return resolve(true);
      }
      const started = performance.now();
      ...
      const frame = now => {
        ...
        if (raw < 1) requestAnimationFrame(frame); else resolve(true);
      };
    ...
    const createTravelOverlay = relation => {
      ...
      shell.style.setProperty('--travel-shift-x', `${(-relation.vector.x * 8).toFixed(2)}vw`);
      shell.style.setProperty('--travel-shift-y', `${(-relation.vector.y * 8).toFixed(2)}vh`);
    ...
    const finishTravel = (relation, transitionToken, result = 'completed') => {
      ...
      state.result = result;
      scene?.transitions?.finish?.(transitionToken, { kind: 'cross-link', sourceId: relation.sourceId, targetId: relation.targetId, relationType: relation.type, result });
      renderRail();
    ...
    const navigate = async relationInput => {
      ...
      const relation = relationsFor(sourceId).find(item => item.targetId === targetId && (!relationInput?.type || item.type === relationInput.type));
      if (!relation) return false;

      const transitionToken = scene?.transitions?.begin?.({ kind: 'cross-link', sourceId: relation.sourceId, targetId: relation.targetId, relationType: relation.type, direction: relation.direction, vector: relation.vector }) || null;
    ...
    detail?.addEventListener('click', event => {
      ...
      const relation = relationsFor(sourceId).find(item => (item.target.detailLabel || item.target.label) === targetLabel || item.target.label === targetLabel);
      if (!relation) return;
      event.preventDefault();
    ...
    new MutationObserver(() => scheduleRender()).observe(document.documentElement, { attributes: true, attributeFilter: ['data-profile-intro'] });
    ...
    window.ProfileCrossLinkTravel = Object.freeze({
      ...
      snapshot: () => ({ ...state, vector: state.vector ? { ...state.vector } : null, currentSourceId: currentSourceId(), railVisible: !rail.hidden, relationCount: railList.children.length, overlayPresent: Boolean(overlay?.shell) })
    });

    scheduleRender();
  };

  if (window.ProfileGlobalGeometry) {
    start();
    return;
  }
  const script = document.createElement('script');
  script.src = 'global-geometry.js';
  script.async = false;
  script.dataset.profileGlobalGeometryLoader = 'true';
