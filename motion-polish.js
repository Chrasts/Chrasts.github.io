(() => {
  ...
  const morphRootToIdentity = shell => {
    ...
    Promise.allSettled([dotAnimation.finished, identityAnimation.finished]).then(() => {
      ...
      shell.classList.add('is-root-morph-complete');
    });
  };
  ...
  const beginLabelInterpolation = context => {
    ...
    overlay.querySelectorAll('.site-graph-node[data-node-id]').forEach(cloneNode => {
      ...
      ['.site-graph-label', '.site-graph-meta'].forEach(selector => {
        ...
        const from = {
          centerX: Number(cloneText.dataset.motionFromX),
          y: Number(cloneText.dataset.motionFromY)
        };
        ...
        records.push({
          element: cloneText,
          baseTransform: cloneText.dataset.motionBaseTransform || '',
          dx,
          dy
        });
      });
  ...
  const scheduleLabelInterpolation = context => {
    requestAnimationFrame(() => requestAnimationFrame(() => requestAnimationFrame(() => {
      beginLabelInterpolation(context);
    })));
  };
  ...
  const captureNavigation = event => {
    ...
    /* graph-transitions-v6 registers its capture listener before this runtime,
       so its overlay already exists at this point. */
    if (document.body.classList.contains('is-v9-transitioning')) {
      promoteTransitionTarget(targetId);
    } else {
      requestAnimationFrame(() => promoteTransitionTarget(targetId));
    }
  };
  ...
  const initStructuralPolish = () => {
    ...
    window.addEventListener('hashchange', () => {
      if (!transitionContext || !document.body.classList.contains('is-v9-transitioning')) return;
      scheduleLabelInterpolation(transitionContext);
    });
  };

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initStructuralPolish, { once: true });
  } else {
    initStructuralPolish();
  }
