...
test.describe('Phase 2 root landing — desktop', () => {
  ...
  test('opens on standalone root scene and keeps first-level graph hidden', async ({ page }) => {
    await boot(page);
  ...
  test('once activated, returning to Overview keeps the expanded graph for the session', async ({ page }) => {
    await boot(page);
    await page.locator('.root-node-trigger').click();
