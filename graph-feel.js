(() => {
  ...
  const loadDependency = (src, marker, ready) => new Promise((resolve, reject) => {
  ...
  const start = async () => {
    await loadDependency('halo-renderer.js', 'data-profile-halo-renderer', () => Boolean(window.ProfileHaloRenderer));
    await loadDependency('node-interaction-state.js', 'data-profile-node-interaction', () => Boolean(window.ProfileNodeInteraction));
    if (window.ProfileGraphFeel) return;
  ...
  start().catch(error => {
    console.error('Profile graph interaction failed to initialise.', error);
  });
})();
