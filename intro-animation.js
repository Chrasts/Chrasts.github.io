(() => {
  ...
  const cleanClone = clone => {
    ...
    clone.querySelectorAll('.site-graph-node[data-node-id]').forEach(element => {
      ...
      element.dataset.introSectionId = sectionId;
      element.dataset.introOriginX = String(origin.x);
      element.dataset.introOriginY = String(origin.y);
      element.dataset.introSectionX = String(section.x);
      element.dataset.introSectionY = String(section.y);
      element.dataset.introRootX = String(rootPoint.x);
      element.dataset.introRootY = String(rootPoint.y);
  ...
  const boundsFor = (svg, predicate, padding, fallback) => {
    const points = [...svg.querySelectorAll('.site-graph-node[data-node-id]')]
      .filter(predicate)
      .map(element => ({
        x: Number(element.dataset.introOriginX ?? element.dataset.x),
        y: Number(element.dataset.introOriginY ?? element.dataset.y)
      }))
  ...
  const viewBoxTargets = svg => {
    ...
    const rootX = Number(root?.dataset.introOriginX ?? root?.dataset.x);
    const rootY = Number(root?.dataset.introOriginY ?? root?.dataset.y);
    const rootWidth = Math.min(760, full.width * .34);
  ...
  const animateNodeCollapse = (svg, stage, duration, id) => new Promise(resolve => {
    const records = [...svg.querySelectorAll('.site-graph-node[data-node-id]')].map(element => ({
      element,
      from: pointOf(element),
      to: collapseTarget(element, stage)
    }));
    if (reducedMotion || duration <= 0) {
      records.forEach(record => setPoint(record.element, record.to));
      resolve(true);
      return;
    }
    const started = performance.now();
  ...
  const profileTags = () => String(profile.label || 'Data analysis · Research · Mathematical logic')
  ...
  const buildIdentityNode = () => {
    ...
    anchor.className = 'profile-intro-identity-anchor';
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'profile-intro-identity';
    button.setAttribute('aria-label', 'Open the profile map');
    const portrait = document.createElement('span');
    portrait.className = 'profile-intro-identity-portrait';
    ...
    name.className = 'profile-intro-identity-name';
    name.textContent = profile.name || 'Štěpán Chrast';
    const hint = document.createElement('span');
    hint.className = 'profile-intro-identity-hint';
    hint.textContent = 'Open profile map';
    const orbit = document.createElement('span');
    orbit.className = 'profile-intro-identity-orbit';
    profileTags().forEach((tag, index) => {
      const item = document.createElement('span');
      item.className = `profile-intro-identity-tag is-${index + 1}`;
      item.textContent = tag;
      orbit.appendChild(item);
    });
    button.append(portrait, name, hint, orbit);
    anchor.appendChild(button);
    overlay.appendChild(anchor);
    identityAnchor = anchor;
    button.addEventListener('click', openProfile);
    requestAnimationFrame(() => anchor.classList.add('is-visible'));
    return anchor;
  };
  ...
  const buildOverlay = sourceSvg => {
    ...
    enter.type = 'button';
    enter.className = 'profile-intro-enter';
    enter.innerHTML = '<span>Enter profile</span><small>Condense the Atlas</small>';
    enter.setAttribute('aria-label', 'Enter profile and condense the Atlas');
    enter.addEventListener('click', startCondensation);
    ...
    shell.append(surface, enter, skip, caption);
  ...
  const skipToLanding = async () => {
    ...
    await wait(reducedMotion ? 80 : 150);
  ...
  const failSafe = reason => {
    ...
    if (!finalising) skipToLanding();
  };

  async function startCondensation() {
    if (!state.eligible || state.stage !== 'atlas' || state.running || finalising || !introSvg) return false;
    state.running = true;
    state.waiting = false;
    if (enterControl) enterControl.disabled = true;
    overlay?.classList.add('is-condensing');
    document.documentElement.dataset.profileIntro = 'running';
    emit('started');
    track('intro_started');
    const id = ++runId;
    const targets = viewBoxTargets(introSvg);

    if (reducedMotion) {
      document.documentElement.dataset.profileIntro = 'identity';
      setStage('identity');
      setViewBox(introSvg, targets.root);
      buildIdentityNode();
      markSeen();
      state.running = false;
      await wait(30);
      identityAnchor?.querySelector('button')?.focus?.({ preventScroll: true });
      return true;
    }

    setStage('territories');
    await Promise.all([
      animateViewBox(introSvg, targets.territories, 820, id),
      animateNodeCollapse(introSvg, 'territories', 820, id)
    ]);
    if (id !== runId || finalising) return false;
    setStage('branches');
    await Promise.all([
      animateViewBox(introSvg, targets.branches, 800, id),
      animateNodeCollapse(introSvg, 'branches', 800, id)
    ]);
    if (id !== runId || finalising) return false;
    setStage('root');
    await Promise.all([
      animateViewBox(introSvg, targets.root, 900, id),
      animateNodeCollapse(introSvg, 'root', 900, id)
    ]);
    if (id !== runId || finalising) return false;
    buildIdentityNode();
    document.documentElement.dataset.profileIntro = 'identity';
  ...
  async function openProfile() {
    ...
    state.running = true;
    setStage('expanding');
    document.documentElement.dataset.profileIntro = 'expanding';

    if (!window.ProfileRootLanding?.isActive?.()) {
      const overviewReady = await internalRoute('overview');
      if (!overviewReady) {
        finalising = false;
        return failSafe('overview-before-expand-timeout');
      }
      await waitFor(() => window.ProfileRootLanding?.isActive?.() === true, 2200);
    }
    ...
    if (!graphReady) {
      finalising = false;
      return failSafe('expanded-root-timeout');
    }

    const realRoot = document.querySelector(`#site-graph .site-graph-node[data-node-id="${rootId}"]`);
    const realDot = realRoot?.querySelector('.site-graph-dot') || realRoot;
    const targetRect = realDot?.getBoundingClientRect();
    const sourceRect = identityAnchor.getBoundingClientRect();
    if (!targetRect || !sourceRect.width || !sourceRect.height) {
      finalising = false;
      return failSafe('root-geometry-missing');
    }

    const dx = targetRect.left + targetRect.width / 2 - (sourceRect.left + sourceRect.width / 2);
    const dy = targetRect.top + targetRect.height / 2 - (sourceRect.top + sourceRect.height / 2);
    const scale = Math.max(.1, Math.min(.34, targetRect.width / sourceRect.width));
    overlay?.classList.add('is-expanding-profile');
    const animation = identityAnchor.animate([
      { transform: 'translate(0px, 0px) scale(1)', opacity: 1 },
      { transform: `translate(${dx}px, ${dy}px) scale(${scale})`, opacity: .96 }
    ], {
    ...
    await Promise.race([
      ...
      wait(reducedMotion ? 200 : 820)
    ]);

    setUnderlyingInert(false);
    restoreInitialOverviewURL();
    completeState('completed');
    realRoot?.focus?.({ preventScroll: true });
    finalising = false;
    return true;
  }
  ...
  const prepare = async id => {
    ...
    if (id !== runId || finalising) return;
    state.waiting = true;
    document.documentElement.dataset.profileIntro = 'waiting';
    emit('ready');
    track('intro_ready');
  };

  window.ProfileIntro = Object.freeze({
    start: startCondensation,
    skip: skipToLanding,
    openProfile,
    snapshot: () => ({
      ...state,
