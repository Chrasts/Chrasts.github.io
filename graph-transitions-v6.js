(() => {
  ...
  const routeNode = route => route === 'overview' ? rootNode : graph.nodes.find(node => node.route === route) || null;
  ...
  const edgePath = (from, to, key, straight = false) => {
    if (!from || !to) return '';
    if (straight || Math.abs(from.x - to.x) < 4) return `M ${from.x.toFixed(1)} ${from.y.toFixed(1)} L ${to.x.toFixed(1)} ${to.y.toFixed(1)}`;
  ...
  const enforceSpacing = (items, left, right, gap) => {
    ...
    for (let i = 1; i < items.length; i += 1) items[i].x = Math.max(items[i].x, items[i - 1].x + gap);
    if (items.at(-1).x > right) {
      const shift = items.at(-1).x - right;
      items.forEach(item => item.x -= shift);
    }
    for (let i = items.length - 2; i >= 0; i -= 1) items[i].x = Math.min(items[i].x, items[i + 1].x - gap);
    if (items[0].x < left) {
  ...
  const arrangeDirect = (nodes, current) => {
    ...
    const items = nodes.map((node, index) => {
      ...
      if (nodes.length === 1) x = current.x + (stableNumber(node.id) % 2 ? 128 : -128);
      else if (Math.abs(x - current.x) < 96) x += stableNumber(`${node.id}:fan`) % 2 ? 112 : -112;
      return { id: node.id, node, x, y: 448 + (index % 2 ? 11 : -9) };
    });
    return enforceSpacing(items, 145, 1055, nodes.length > 5 ? 132 : 166);
  };

  const arrangeSecondRank = (directNodes, directItems, visibleIds) => {
    const directPosition = new Map(directItems.map(item => [item.id, item]));
    const secondMap = new Map();

    directNodes.forEach(parent => {
      childrenFor(parent.id).filter(child => visibleIds.has(child.id)).forEach(child => {
    ...
    const items = [...secondMap.values()].map(({ node, parentIds }, index) => {
      ...
      const barycentre = parentXs.length ? parentXs.reduce((sum, value) => sum + value, 0) / parentXs.length : 600;
      let x = barycentre;
      if (parentXs.length === 1) x += stableNumber(`${node.id}:branch`) % 2 ? 76 : -76;
      return { id: node.id, node, x, y: 610 + (index % 2 ? 8 : -8), barycentre };
    });

    items.sort((a, b) => a.barycentre - b.barycentre || a.node.label.localeCompare(b.node.label));
    return enforceSpacing(items, 125, 1075, items.length > 6 ? 126 : 146);
  };
  ...
  const reflowFocus = () => {
    ...
    const direct = childrenFor(target.id).filter(node => visibleIds.has(node.id)).sort((a, b) => a.label.localeCompare(b.label));
    const directItems = arrangeDirect(direct, current);
    directItems.forEach(item => positions.set(item.id, { x: item.x, y: item.y }));

    arrangeSecondRank(direct, directItems, visibleIds)
      .filter(item => !positions.has(item.id))
      .forEach(item => positions.set(item.id, { x: item.x, y: item.y }));

    const unplaced = [...visibleIds].filter(id => !positions.has(id)).map(id => nodeMap.get(id)).filter(Boolean).sort((a, b) => a.label.localeCompare(b.label));
    ...
    enforceSpacing(fallback, 130, 1070, fallback.length > 6 ? 122 : 142).forEach(item => positions.set(item.id, { x: item.x, y: item.y }));
  ...
  const semanticPersistentIds = (current, finalTargetId) => {
    const target = nodeMap.get(finalTargetId) || currentRouteNode(current.targetRoute);
    ...
    const from = currentRouteNode(current.currentRoute);
    const fromIds = new Set(primaryPath(from).map(node => node.id));
    return new Set(primaryPath(target).filter(node => fromIds.has(node.id)).map(node => node.id));
  };
  ...
  const outwardPoint = (from, anchor, id) => {
    ...
    const distance = 54 + (stableNumber(`${id}:out`) % 20);
    return {
      x: from.x + Math.sign(dx) * distance * (.78 + Math.abs(dx / length) * .22),
      y: from.y + (dy / length) * distance * .20 - 4
    };
  };
  ...
  const startTransition = () => {
    ...
    const persistentIds = semanticPersistentIds(current, finalTargetId);

    const persistent = [];
    const leaving = [];
    current.before.forEach((item, id) => {
      if (after.has(id) && persistentIds.has(id)) persistent.push({ id, element: item.clone, from: item.point, to: after.get(id) });
      else leaving.push({ id, element: item.clone, from: item.point });
    });
    ...
    const entering = [];
    after.forEach((to, id) => {
      if (persistentIds.has(id) && current.before.has(id)) return;
      ...
      const origin = current.direction === 'down' ? (targetBefore || targetAfter) : targetAfter;
      setPoint(clone, origin);
      current.overlayNodes.appendChild(clone);
      entering.push({ id, element: clone, from: origin, to });
    });

    const transitionEdges = edgeElements().map(source => {
      const clone = source.cloneNode(true);
      clone.style.opacity = '0';
    ...
    [...camera.querySelectorAll(':scope > .site-graph-decorations > *')].forEach(source => {
      const clone = source.cloneNode(true);
      clone.style.opacity = '0';
      clone.style.pointerEvents = 'none';
    ...
    const duration = current.direction === 'up' ? 1180 : current.direction === 'down' ? 1080 : 980;
    ...
    const frame = now => {
      ...
      const movingTarget = targetPersistent ? lerpPoint(targetPersistent.from, targetPersistent.to, moveP) : targetAfter;

      if (current.direction === 'up') {
        const collapseRaw = clamp01(raw / .65);
        const collapseP = ease(collapseRaw);
        leaving.forEach(item => {
          setPoint(item.element, lerpPoint(item.from, movingTarget, collapseP));
          const fade = clamp01((collapseRaw - .60) / .40);
          item.element.style.opacity = String(1 - ease(fade));
        });
      } else if (current.direction === 'down') {
        const fadeRaw = clamp01(raw / .36);
        leaving.forEach(item => {
      ...
      const enterStart = current.direction === 'up' ? .58 : current.direction === 'down' ? .24 : .34;
      const enterRaw = clamp01((raw - enterStart) / (1 - enterStart));
      const enterP = ease(enterRaw);

      entering.forEach(item => {
        const origin = current.direction === 'down' || current.direction === 'up' ? movingTarget : item.from;
      ...
      const edgeStart = current.direction === 'up' ? .64 : current.direction === 'down' ? .36 : .48;
      ...
      const decorationRaw = clamp01((raw - .60) / .40);
      [...current.overlayDecorations.children].forEach(element => element.style.opacity = String(ease(decorationRaw)));
  ...
  const scheduleTransition = () => requestAnimationFrame(() => requestAnimationFrame(startTransition));
  ...
  document.addEventListener('click', event => {
    ...
    prepare({ targetId: node?.dataset.nodeId || routeTargetId(route), targetRoute: route, trigger: 'click' });
  }, true);
  ...
  document.addEventListener('keydown', event => {
    if (event.key === 'Enter' || event.key === ' ') {
      ...
      if (route === currentRoute) return;
      const node = event.target.closest?.('.site-graph-node[data-node-id]');
      prepare({ targetId: node?.dataset.nodeId || routeTargetId(route), targetRoute: route, trigger: 'keyboard' });
      return;
    }
    ...
    if (event.key === 'Escape') {
      ...
      const target = mode === 'work' ? rootNode : nodeMap.get(current?.parentIds?.[0]) || rootNode;
